...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let bd = process.env.MEDUSA_BACKEND_URL, be = { regionMap: /* @__PURE__ */ new Map(), regionMapUpdated: Date.now() };
      async function bf(a10) {
        let { regionMap: b2, regionMapUpdated: c2 } = be;
        if (!bd) throw Error("Middleware.ts: Error fetching regions. Did you set up regions in your Medusa Admin and define a MEDUSA_BACKEND_URL environment variable? Note that the variable is no longer named NEXT_PUBLIC_MEDUSA_BACKEND_URL.");
        if (!b2.keys().next().value || c2 < Date.now() - 36e5) {
          let { regions: b3 } = await fetch(`${bd}/store/regions`, { headers: { "x-publishable-api-key": "pk_fac1ae55ae2ee7d1983c9397657644629472c68170caf76259d32cd9ab613107" }, next: { revalidate: 3600, tags: [`regions-${a10}`] }, cache: "force-cache" }).then(async (a11) => {
            let b4 = await a11.json();
            if (!a11.ok) throw Error(b4.message);
            return b4;
          });
          if (!b3?.length) throw Error("No regions found. Please set up regions in your Medusa Admin.");
          b3.forEach((a11) => {
            a11.countries?.forEach((b4) => {
              be.regionMap.set(b4.iso_2 ?? "", a11);
            });
          }), be.regionMapUpdated = Date.now();
        }
        return be.regionMap;
      }
      async function bg(a10, b2) {
        try {
          let c2, d2 = a10.headers.get("x-vercel-ip-country")?.toLowerCase(), e2 = a10.nextUrl.pathname.split("/")[1]?.toLowerCase();
          return e2 && b2.has(e2) ? c2 = e2 : d2 && b2.has(d2) ? c2 = d2 : b2.has("us") ? c2 = "us" : b2.keys().next().value && (c2 = b2.keys().next().value), c2;
        } catch (a11) {
        }
      }
      async function bh(a10) {
        let b2 = a10.nextUrl.href, c2 = R.redirect(b2, 307), d2 = a10.cookies.get("_medusa_cache_id"), e2 = d2?.value || crypto.randomUUID(), f2 = await bf(e2), g2 = f2 && await bg(a10, f2), h2 = g2 && a10.nextUrl.pathname.split("/")[1].includes(g2);
        if (h2 && d2) return R.next();
        if (h2 && !d2) return c2.cookies.set("_medusa_cache_id", e2, { maxAge: 86400 }), c2;
        if (a10.nextUrl.pathname.includes(".")) return R.next();
        let i2 = "/" === a10.nextUrl.pathname ? "" : a10.nextUrl.pathname, j2 = a10.nextUrl.search ? a10.nextUrl.search : "";
        return !h2 && g2 && (b2 = `${a10.nextUrl.origin}/${g2}${i2}${j2}`, c2 = R.redirect(`${b2}`, 307)), c2;
      }
      let bi = { matcher: ["/((?!api|_next/static|_next/image|favicon.ico|images|assets|png|svg|jpg|jpeg|gif|webp).*)"] };
      Object.values({ NOT_FOUND: 404, FORBIDDEN: 403, UNAUTHORIZED: 401 });
      let bj = { ...e }, bk = bj.middleware || bj.default, bl = "/src/middleware";
      ...
      function bm(a10) {
        return bb({ ...a10, page: bl, handler: async (...a11) => {
          try {
            return await bk(...a11);
          } catch (e2) {
